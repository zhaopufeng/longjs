...
export default function validateParams(data: { [key: string]: any }, validateKeys: ValidationRule): { [key: string]: any } {
    const errors: { [key: string]: any } = {}
    Object.keys(validateKeys).forEach((key: string) => {
        // Skip validate
        if (!validateKeys[key]) return;
...
export type Messages<S = string, P = ParamsRules> = { [K in keyof P]: S }
